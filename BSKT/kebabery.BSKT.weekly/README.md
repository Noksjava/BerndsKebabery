...
Windows & .NET 8 SDK (or VS with .NET8) / MS Edge WebView2 Runtime (for API fetch)
...
- **Fetch via Alvara API**: fetches from Alvara API URL (configurable) and builds payload from API docs.
...
- If **Save fetched result as JSON1** is enabled:
  - app prompts for save location and writes a `.json1` (.json + top 5 leaderboard payload)
